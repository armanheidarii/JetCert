...
class CD:
    ...
    def create_updates(self, changed_files):
        if changed_files == None:
            raise ValueError(f"The changed_files {changed_files} cannot be empty!")

        if type(changed_files) != list:
            raise ValueError(f"The changed_files {changed_files} is not list!")
    ...
    def put_update(self, update):
        if not update:
            raise ValueError(f"The update {update} cannot be empty!")
    ...
    def put_updates(self, updates):
        if updates == None:
            raise ValueError(f"The updates {updates} cannot be empty!")

        if type(updates) != list:
            raise ValueError(f"The updates {updates} is not list!")
    ...
    def update(self):
        ...
        if is_git_pull_invalid:
            raise ValueError(f"The is_git_pull {is_git_pull_invalid} is not valid!")
