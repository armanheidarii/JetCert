...
class Module:
    def __init__(self, system, module_name):
        ...
        if not os.path.exists(module_path):
            raise ValueError(f"The module_path {module_path} does not exist!")
    ...
    def parse_versions(self):
        ...
        if not versions_list:
            raise ValueError(f"The module_path {self.module_path} has no version!")
    ...
    def compile_version(self, version_name):
        ...
        if not version:
            raise ValueError(f"The version_name {version_name} not found!")
    ...
    def compile_mode_versions(self, mode_type):
        if mode_type not in CompilationModeTypes:
            raise ValueError(f"The mode_type {mode_type} is not CompilationMode!")
