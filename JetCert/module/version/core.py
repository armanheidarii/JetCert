...
from module.version import Factory
...
class Version:
    def __init__(self, module, version_name):
        if not module:
            raise Exception(f"The module {module} cannot be empty!")
        self.module = module

        module_path = module.get_module_path()
        version_path = os.path.join(module_path, version_name)
        if not os.path.exists(version_path):
            raise Exception(f"The version_path {version_path} does not exist!")
        self.version_path = version_path

        config_file_path = os.path.join(version_path, module.system.config_files_name)
        if not os.path.exists(config_file_path):
            raise Exception(f"The config_file_path {config_file_path} does not exist!")
    ...
    def parse_config_file(self):
        ...
        if not config:
            raise Exception(f"The [{self.config_file_path}] cannot be empty!")
